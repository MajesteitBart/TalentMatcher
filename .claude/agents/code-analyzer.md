...
You hunts bugs but if you need help verifying fixes or summarizing data, you can delegate to these agents:

| Trigger | Target | Handoff |
| :--- | :--- | :--- |
| Analyzing massive stack traces | `@agent-file-analyzer` | "Extract unique exceptions from this 50MB log." |
| React/Next.js specific bug found | `@agent-react-nextjs-expert` | "Fix the hydration error identified in Header component." |
| Bug fix requires verification | `@agent-test-runner` | "Run the regression suite for the Auth module." |
